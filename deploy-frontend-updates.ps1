...
$copyCommand = "ssh -i `"$KEY_PATH`" -o StrictHostKeyChecking=no $SERVER `"sudo cp $PROJECT_DIR/architecture-and-techstack.html /var/www/html/ && sudo cp $PROJECT_DIR/tech-stack-visualization.html /var/www/html/ && sudo cp $PROJECT_DIR/architecture-visualization.html /var/www/html/ && sudo chown www-data:www-data /var/www/html/*.html`""
...
Write-Host "   - Use browser's print function (Ctrl+P) to save as PDF" -ForegroundColor White
