...
# CyberShield AI - AWS EC2 Deployment Script (Fixed Version)
# Handles Ubuntu version compatibility and APT lock issues

set -e  # Exit on error

echo "🚀 CyberShield AI - AWS EC2 Deployment (Fixed)"
echo "=============================================="
...
for i in 1 2 3; do
    if sudo fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1; then
        print_warning "Waiting for other apt process to finish... (attempt $i/3)"
        sleep 10
    else
        break
    fi
...
# Install prerequisites (using available Python version)
...
try {
    ...
    if (e.message.includes('already exists')) {
        ...
        print('Error creating MongoDB user: ' + e.message);
    }
}
"
...
sudo cp /home/ubuntu/CyberShield-AI-/nginx-direct.conf /etc/nginx/sites-available/cybershield
...
try {
    ...
    print('Database initialization error: ' + e.message);
}
"
